...
class VideoBufferWriter():
    ...
    def __init__(
        ...
        self.__meta_queue_time, self.__meta_queue_logic = meta_queue
    ...
    def run(self) -> None:
        ...
        try:
            frames = self.__input_queue
            # time, logic = self.__meta_queue
        except Empty:
            frames = None

        # is_receiving_frames makes sure we don't process the same frame twice.
        # self._configure(frames[0,:,:].shape)
        # self._ingest_frame(frames)
        if frames is not None:
            self._create_video_from_array(frames)
    ...
    def _create_video_from_array(self, array: np.ndarray) -> None:
        raise NotImplementedError("Should be implemented by subclass.")
    ...
    def _complete(self, filename: str) -> None:
        ...
        """
        save_df(
            pd.DataFrame({"t":self.__meta_queue_time,
                          "logic":self.__meta_queue_logic}),
...
class BufferVideoWriter(VideoBufferWriter):
    """
    ...
    def _create_video_from_array(self, array: np.ndarray) -> None:
        """
        Creates a video from a 3D numpy array using PyAV.
    
        Parameters:
        - array (np.ndarray): A 3D numpy array of shape (timepoints, x, y) representing the video frames.
        """
        super()._configure(array[0,:,:].shape)
        ...
        self.timepoints, self._stream.height, self._stream.width = array.shape
        ...
        # This ensures all frames are written
    ...
    def _complete(self, filename: str) -> None:
        ...
        """
        super()._complete(self._get_filename_base())
